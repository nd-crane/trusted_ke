...
def select_samples(result_df, sample_ids, id_col):

    sampled_result_df = pd.DataFrame(columns=result_df.columns)
    for sample_id in sample_ids:
        id_rows = result_df[result_df[id_col]==sample_id]
        sampled_result_df = pd.concat([sampled_result_df, id_rows], ignore_index=True)
...
if __name__=='__main__':
    ...
    sample_ids = list(sample_df['c5'])

    sampled_result_df = select_samples(result_df, sample_ids, args.id_col)

    sampled_result_df.to_excel(args.output_folder + args.dataset_path.split('/')[-1].split('.csv')[0]+'_eval.xlsx', index=False)