...
import transformers
from transformers import PretrainedConfig ################## ADDED
...
class Runner:
    ...
    def load_model_checkpoint(self, wrapper, suffix, continue_training=False):
        ...
        if not os.path.exists(path_ckpt):
            config_ckpt = PretrainedConfig.from_pretrained(suffix) ############# ADDED
            config_ckpt.pretrained_name_or_path = "tliu/asp-coref-t0-3b" ############# ADDED
            config_ckpt._name_or_path = "tliu/asp-coref-t0-3b" ############# ADDED
            wrapper.model = wrapper.model.from_pretrained(config_ckpt.pretrained_name_or_path, config=config_ckpt) ############# ADDED
